...
module.exports = {
  parse: function(packet, callback) {
    ...
    var heading = (-1) * (180/Math.PI) * this.parseSingle(packet.slice(12, 16));
    ...
    for (var i = 0; i < vehiclesData.length / 40; i++) {
        ...
        if (ID != 0) {
            ...
            var heading = (-1) * (180/Math.PI) *this.parseSingle(currentVehicle.slice(12, 16));
	    ...
	    if (ID === 120) { width = ego.width; length = ego.length; }
            var vehicle = {};
